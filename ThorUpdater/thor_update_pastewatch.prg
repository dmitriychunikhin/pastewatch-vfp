*File to control Thor CFU for application pastewatch
lparameters;
	toUpdateObject
...
lcRepositoryURL   = 'https://github.com/dmitriychunikhin/pastewatch-vfp'
...
lcVersionFileURL  = m.lcDownloadsURL + 'pastewatchVersion.txt' 
	&& the URL for the file containing code to get the available version number
lcZIPFileURL      = m.lcDownloadsURL + 'pastewatch.zip'
	&& the URL for the zip file containing the project


text to lcRegisterWithThor noshow textmerge
	? 'Pastewatch is updated. From the Thor menu, choose "More -> Open Folder -> Components" to see the folder where pastewatch was installed'
endtext

* Set the properties of the passed updater object.

with m.toUpdateObject
	.ApplicationName      = 'pastewatch'
	.VersionLocalFilename = 'pastewatchVersionFile.txt'
	.VersionFileURL       = m.lcVersionFileURL
	.SourceFileUrl        = m.lcZIPFileURL
	.Component            = 'Yes'
	.Link                 = m.lcRepositoryURL
	.LinkPrompt           = 'pastewatch github'
	.ProjectCreationDate  = date(2025, 2, 18)
